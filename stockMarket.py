...
class Stock:
	...
	sample_data.csv for an example and StockTest.py for test cases.
	"""
	def __init__(self, csv_file):
		self.data= self._read_stock_data(csv_file)
	...
	def _read_stock_data(self, csv_file):
		...
		data['Fixed_Dividend']= fixed_dividend
		return data

	def get_dividend_yield(self, stock, ticker_price):
		"""Answers the exercise question 'Calculate the dividend yield'
		"""
		dstock= self.data[self.data['Stock_Symbol'] == stock]
	...
	def get_pe_ratio(self, stock, ticker_price):
		"""Answers question 'Calculate the P/E Ratio'
		"""
	...
	def record_trade(self, stock, quantity, sold, price):
		"""Answers question 'Record a trade, with timestamp, quantity of
		...
		bought
		"""
		if stock not in set(self.data['Stock_Symbol']):
	...
	def get_stock_price(self, stock, last_minutes= 15):
		"""Answers question 'Calculate Stock Price based on trades
	...
	def get_all_share_index(self):
		"""Answers question 'Calculate the GBCE All Share Index using the geometric mean of prices for all stocks'
		"""
		if len(self.trade) == 0:
			return float('nan')
		all_share_index= 1
